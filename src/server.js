  ...
  for (let i = 1; i < sections.length; i++) {
    ...
    if (title && content) {
      fields.push({
        name: title,
        value:
          content.length > 1024 ? content.substring(0, 1021) + "..." : content,
        inline: false,
      });
    }
  }
...
async function processPersonDailySummary(interaction, env) {
  ...
  const date =
    commandOptions.find((o) => o.name === "date")?.value ||
  ...
  try {
    ...
    const embedPayload = parseSummaryToEmbed(
      personName,
      date,
      text,
      env.WORKSPACE_SLUG
...
async function sendMessageToChannel(channelId, discordToken, payload) {
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      logger.error(
        `Failed to send channel message: ${response.status} ${errorText}`
      );
      return false;
    }
    return true;
  ...
  try {
    ...
    for (const member of members) {
      ...
      try {
        ...
        const embedPayload = parseScheduledSummaryToEmbed(
          displayName,
          today,
          text,
          env.WORKSPACE_SLUG
        ...
        logger.error(
          `Error generating scheduled summary for ${displayName}: ${error.message}`
        );
    ...
    logger.info(
      `Scheduled summary complete: ${summariesSent} summaries sent, ${noActivityCount} with no activity`
    );
  } catch (error) {
